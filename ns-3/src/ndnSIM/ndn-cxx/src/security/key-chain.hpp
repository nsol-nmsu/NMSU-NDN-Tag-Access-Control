...
/**
* @class KeyChain
* This class has been modified to accomodate the modifications to Interest and
* Data classes and to accomodate signing AuthTags for NMSU's tag based access
* control mechanism.
**/
...
public:
  ...
  /**
   * @brief Sign a AuthTag, Interest, or Data object
   *
   ...
   *
   * @param packet AuthTag, Data, or Interest to sign
  ...
  template<typename T>
  void
  sign( T& packet, const SigningInfo& params = DEFAULT_SIGNING_INFO);
   ...
   * @brief Sign any object that implements 'wireEncode( EncodingBuffer<...> ... )' and
   *        'const Block& wireEncode()' methods and have a tlv::SignatureValue terminating
   *        the signed portion of their encodings.
   *
...
KeyChain::signImpl(T& packet, const SigningInfo& params)
{
  ...
  const Block& signed_block = packet.wireEncode().get( tlv::SignedPortion );

  Block sigValue = pureSign( signed_block.value(),
                            signed_block.value_size(),
                            keyName,
                            params.getDigestAlgorithm() );
  packet.setSignatureValue( sigValue );
...
template<typename T>
void
KeyChain::sign( T& packet, const SigningInfo& params)
{
    signImpl( packet, params );
}
