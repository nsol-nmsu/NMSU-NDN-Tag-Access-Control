...
template< class T >
bool
Validator::verifySignature(const T& packet, const PublicKey& publicKey)
{
  if (!packet.getSignature().hasKeyLocator())
    return false;

  const Block& signed_block = packet.wireEncode().get( tlv::SignedPortion );

  return verifySignature(signed_block.value(),
                         signed_block.value_size(),
                         packet.getSignature(), publicKey );
}

template bool
Validator::verifySignature< AuthTag >(const AuthTag& packet, const PublicKey& publicKey);
template bool
Validator::verifySignature< Data >(const Data& packet, const PublicKey& publicKey);
template bool
Validator::verifySignature< Interest >(const Interest& packet, const PublicKey& publicKey);
