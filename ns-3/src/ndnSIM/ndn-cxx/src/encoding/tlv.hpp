 ...
 * to add types specific to NMSU's tag based access control mechanism
 */
...
enum {
  ...
  /** A wrapper in both Data and Interest packets.  Wrapps all fields that
  *   should be signed.  Any fields outside the wrapper are not signed and
  *   might be modified by the network.
  */
  SignedPortion = 33,
  ...
  *   the edge router sets this probability based on the contents
  *   of its authentication caches ( bloom filters ).  Probability
  *   ranges from 0, indicating that the edge rotuer knows nothing
